...
func NewAddressHandler(logger *logging.Logger, repo *db.AddressRepository, ctx context.Context) handlers.Handler {
...
func (a addressHandler) GetOne(w http.ResponseWriter, r *http.Request) error {
	...
	one, err := a.repo.FindOne(a.ctx, id)
	if err != nil {
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return err
	}
...
func (a addressHandler) Create(w http.ResponseWriter, r *http.Request) error {
	...
	add, err := a.repo.Create(a.ctx, &addr)
	if err != nil {
		w.WriteHeader(http.StatusInternalServerError)
		return err
	}
...
func (a addressHandler) Update(w http.ResponseWriter, r *http.Request) error {
	...
	_, err = a.repo.Update(a.ctx, id, addr)
	if err != nil {
		return err
	}
	w.WriteHeader(http.StatusOK)
